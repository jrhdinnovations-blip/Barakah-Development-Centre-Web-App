...
type Tab =
  | "overview" | "applications" | "bookings" | "enquiries" | "courses" | "templates"
  | "finance" | "cases" | "travel" | "market" | "volunteers" | "swift";
...
function StaffDashboard() {
  ...
  const roles = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("user_roles").select("role").eq("user_id", user!.id).eq("status", "active");
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        {tabBtn("enquiries", "Enquiries (CRM)")}
        {tabBtn("courses", "Courses")}
        {tabBtn("templates", "Notifications")}
        {tabBtn("finance", "Finance")}
        {tabBtn("cases", "Cases")}
        {tabBtn("travel", "Travel")}
        {tabBtn("market", "Marketplace")}
        {tabBtn("volunteers", "Volunteers")}
        {tabBtn("swift", "Swift Move")}
      </div>
  ...
  function OverviewTab({ data, exec }: { data: any; exec?: any }) {
    const stats = [
      ...
      { icon: Activity, label: "Revenue (all time)", value: exec ? formatNaira(exec.totalRevenueKobo) : "—" },
      { icon: Inbox, label: "Active cases", value: exec?.activeCases ?? "—" },
      { icon: CalendarDays, label: "Travel enrolments", value: exec?.activeTravelEnrolments ?? "—" },
            ...
            {(data?.recentApplications ?? []).map((a: any) => (
              <li key={a.id} className="flex justify-between border-b border-border pb-2">
                <span className="text-foreground">
                  ...
                  {Array.isArray(a.programme) ? a.programme[0]?.title : (a.programme?.title ?? "General")}
                </span>
    ...
    return (
      <div className="mt-8">
        <select value={status} onChange={(e) => setStatus(e.target.value)} className={`${inputCls} max-w-xs`}>
          ...
          {["submitted", "under_review", "approved", "rejected", "waitlisted", "withdrawn"].map((s) => (
  ...
  function BookingsTab() {
    ...
    const [svc, setSvc] = useState({ title: "", category: "", durationMinutes: 60, cancelCutoffHours: 24 });
    const [slot, setSlot] = useState({ serviceId: "", startsAt: "", endsAt: "", capacity: 1, location: "" });
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          {(bookings.data ?? []).map((b: any) => {
            ...
            const svcData = Array.isArray(slotData?.service) ? slotData.service[0] : slotData?.service;
            const booker = Array.isArray(b.booker) ? b.booker[0] : b.booker;
            return (
              <div key={b.id} className="card-surface flex flex-wrap items-center justify-between gap-3 p-4">
                <div>
                  <p className="font-medium text-foreground">
                    {svcData?.title} — {booker?.full_name || "User"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {slotData ? fmt(slotData.starts_at) : ""} {slotData?.location ? `· ${slotData.location}` : ""}
                  </p>
                </div>
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <input placeholder="Title" value={svc.title} onChange={(e) => setSvc({ ...svc, title: e.target.value })} required className={inputCls} />
            <input placeholder="Category (e.g. counselling)" value={svc.category} onChange={(e) => setSvc({ ...svc, category: e.target.value })} className={inputCls} />
            <div className="flex gap-3">
              <label className="flex-1 text-xs text-muted-foreground">
                Duration (min)
                <input type="number" min={15} value={svc.durationMinutes} onChange={(e) => setSvc({ ...svc, durationMinutes: +e.target.value })} className={inputCls} />
              </label>
              <label className="flex-1 text-xs text-muted-foreground">
                Cancel cutoff (h)
                <input type="number" min={0} value={svc.cancelCutoffHours} onChange={(e) => setSvc({ ...svc, cancelCutoffHours: +e.target.value })} className={inputCls} />
              </label>
            </div>
            <button className="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Save service</button>
          </form>
          ...
          >
            ...
            <select value={slot.serviceId} onChange={(e) => setSlot({ ...slot, serviceId: e.target.value })} required className={inputCls}>
            ...
            <label className="block text-xs text-muted-foreground">
              Starts
              <input type="datetime-local" value={slot.startsAt} onChange={(e) => setSlot({ ...slot, startsAt: e.target.value })} required className={inputCls} />
            </label>
            <label className="block text-xs text-muted-foreground">
              Ends
              <input type="datetime-local" value={slot.endsAt} onChange={(e) => setSlot({ ...slot, endsAt: e.target.value })} required className={inputCls} />
            </label>
            <div className="flex gap-3">
              <input type="number" min={1} value={slot.capacity} onChange={(e) => setSlot({ ...slot, capacity: +e.target.value })} className={inputCls} aria-label="Capacity" />
              <input placeholder="Location" value={slot.location} onChange={(e) => setSlot({ ...slot, location: e.target.value })} className={inputCls} />
            </div>
            <button className="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Save slot</button>
          </form>
        </div>
      </div>
    );
  ...
  function EnquiriesTab() {
    const enquiries = useQuery({ queryKey: ["staff-enquiries"], queryFn: () => staffListEnquiries({ data: {} }) });
    const followUps = useQuery({ queryKey: ["staff-followups"], queryFn: () => staffListFollowUps() });
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          {(enquiries.data ?? []).length === 0 && (
            <p className="text-sm text-muted-foreground">No enquiries yet — the website contact form feeds this queue.</p>
          )}
          {(enquiries.data ?? []).map((en: any) => (
            <div key={en.id} className="card-surface p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  onClick={() => staffAssignEnquiry({ data: { enquiryId: en.id, assignTo: null } }).then(invalidate)}
                ...
                <button
                  onClick={async () => {
                    ...
                    if (u.user) await staffAssignEnquiry({ data: { enquiryId: en.id, assignTo: u.user.id } });
                    invalidate();
                  }}
                  className="rounded-md border border-input px-3 py-1.5 text-xs font-medium hover:bg-accent"
                >
                ...
                <button
                  onClick={() => staffResolveEnquiry({ data: { enquiryId: en.id, resolved: en.status !== "resolved" } }).then(invalidate)}
                  className="rounded-md border border-input px-3 py-1.5 text-xs font-medium hover:bg-accent"
                >
              ...
              {openId === en.id && (
                <EnquiryNotes enquiryId={en.id} note={note} setNote={setNote} fuDate={fuDate} setFuDate={setFuDate} onDone={invalidate} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="card-surface h-fit p-5">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            ...
            {(followUps.data ?? []).map((f: any) => {
              ...
              return (
                <li key={f.id} className="flex items-center justify-between gap-2 border-b border-border pb-2">
                  <div>
    ...
    return (
      <div className="mt-4 rounded-lg border border-border p-4">
        ...
        <div className="mt-3 flex gap-2">
          <input placeholder="Add a note" value={note} onChange={(e) => setNote(e.target.value)} className={inputCls} />
          <button
            onClick={() =>
        ...
        <div className="mt-3 flex items-center gap-2">
          <input type="date" value={fuDate} onChange={(e) => setFuDate(e.target.value)} className={inputCls} />
          <button
            onClick={() =>
  ...
  function CoursesTab() {
    ...
    const [course, setCourse] = useState({ slug: "", title: "", summary: "", selfEnrol: true, status: "draft" as any });
    const [lessonCourseId, setLessonCourseId] = useState("");
    const [lesson, setLesson] = useState({ title: "", contentType: "document" as "video" | "document", videoUrl: "", body: "", sortOrder: 0 });
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <div className="space-y-6">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              staffUpsertCourse({ data: course }).then(() => {
          ...
          >
            ...
            <input placeholder="slug (e.g. intro-to-tailoring)" value={course.slug} onChange={(e) => setCourse({ ...course, slug: e.target.value })} required className={inputCls} />
            <input placeholder="Title" value={course.title} onChange={(e) => setCourse({ ...course, title: e.target.value })} required className={inputCls} />
            <textarea placeholder="Summary" value={course.summary} onChange={(e) => setCourse({ ...course, summary: e.target.value })} rows={2} className={inputCls} />
            <div className="flex items-center gap-4">
              <select value={course.status} onChange={(e) => setCourse({ ...course, status: e.target.value })} className={inputCls}>
                {["draft", "review", "approved", "published", "archived"].map((s) => (
                  <option key={s} value={s}>{s}</option>
                ))}
              </select>
              <label className="flex items-center gap-2 text-sm text-foreground">
                <input type="checkbox" checked={course.selfEnrol} onChange={(e) => setCourse({ ...course, selfEnrol: e.target.checked })} />
                Self-enrol
              </label>
            </div>
            <button className="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Save course</button>
          </form>

          <form
            onSubmit={(e) => {
              e.preventDefault();
              staffUpsertLesson({ data: { courseId: lessonCourseId, ...lesson } }).then(() => {
                toast.success("Lesson saved.");
                setLesson({ title: "", contentType: "document", videoUrl: "", body: "", sortOrder: 0 });
          ...
          >
            ...
            <select value={lessonCourseId} onChange={(e) => setLessonCourseId(e.target.value)} required className={inputCls}>
              <option value="">Select course…</option>
              {(courses.data ?? []).map((c: any) => (
                <option key={c.id} value={c.id}>{c.title}</option>
              ))}
            </select>
            <input placeholder="Lesson title" value={lesson.title} onChange={(e) => setLesson({ ...lesson, title: e.target.value })} required className={inputCls} />
            <div className="flex gap-3">
              <select value={lesson.contentType} onChange={(e) => setLesson({ ...lesson, contentType: e.target.value as any })} className={inputCls}>
                <option value="document">Document</option>
                <option value="video">Video embed</option>
              </select>
              <input type="number" min={0} value={lesson.sortOrder} onChange={(e) => setLesson({ ...lesson, sortOrder: +e.target.value })} className={inputCls} aria-label="Sort order" />
            </div>
            {lesson.contentType === "video" && (
              <input placeholder="Video embed URL (https://…)" value={lesson.videoUrl} onChange={(e) => setLesson({ ...lesson, videoUrl: e.target.value })} required className={inputCls} />
            )}
            <textarea placeholder="Lesson text / document content" value={lesson.body} onChange={(e) => setLesson({ ...lesson, body: e.target.value })} rows={3} className={inputCls} />
            <button className="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Save lesson</button>
          </form>
        </div>
  ...
  function TemplatesTab() {
    const templates = useQuery({ queryKey: ["staff-templates"], queryFn: () => staffListTemplates() });
    const [editing, setEditing] = useState<Record<string, string>>({});
    const save = useMutation({
      mutationFn: (t: any) =>
        staffUpsertTemplate({
          data: { id: t.id, key: t.key, channel: t.channel, subject: t.subject ?? undefined, body: editing[t.id] ?? t.body },
        }),
      onSuccess: () => {
    ...
    return (
      <div className="mt-8 space-y-4">
        <p className="text-sm text-muted-foreground">
          Use placeholders like {"{{user_name}}"}, {"{{programme_name}}"}, {"{{service_name}}"}, {"{{slot_time}}"}, {"{{course_name}}"}, {"{{status}}"}.
        </p>
